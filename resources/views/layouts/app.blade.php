...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-gray-900 text-gray-200 flex flex-col sticky top-0 h-screen overflow-y-auto">
        ...
        <!-- Menú -->
        <nav class="flex-1 p-4 space-y-2" x-data="{ active: null }">
        ...
        {{-- Reportes (solo admin, pendiente) --}}
        @if($rol === 'Administrador')
        <div>
            <button class="w-full text-left px-4 py-2 rounded text-gray-500 cursor-not-allowed flex justify-between items-center">
                Reportes
                <span class="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">Pronto</span>
            </button>
        </div>
        @endif
